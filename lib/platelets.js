...
function props(dest, pds, src, isrc, ndflt, nm, nmsrc) {
    ...
    while (i--) {
        if (asrt) prop(dest, pds[i], src, isrc, ndflt, nm, nmsrc);
            ...
            if (src) prop(src, pds[i], false, true);
            prop(dest, pds[i], src, isrc, ndflt);
        }
    }
    return dest;
}
...
function prop(dest, pd, src, isrc, ndflt, nm, nmsrc) {
    if (nm && src) {
        ...
        if (typ === 'undefined') v = (src && (isrc || pd.i) && typeof src[pd.n] !== 'undefined' ? src[pd.n] : undefined) || (ndflt ? undefined : pd.d);
