...
Drip.prototype.emit = function emit(pw, iv) {
    ...
    var pulse = new Pulse(pw, iv, drip), sy = drip.pulse.type === 'sync', m = sy ? pw.emit : pw.emitAsync, isPass = sy || drip.pulse.event === pw.options.endEvent;
    ...
    if (argsp || argsg) {
        ...
        if (isPass) iv.pass.length = 0; // auto-clear passed parameters
        m.apply(pw, args);
