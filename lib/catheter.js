...
function catheter(pw, opts, args) {
    ...
    iv.pump = function pump(meds, begin) {
        ...
        typed(merge(iv, artery(iv, meds), true));
    ...
    iv.drain = function drain(drip) {
        ...
        if (!isEnd && isLast) {
            ...
            for (var ci = iv.indexOf(drip), i = ci, x = -1, cnt = iv.count; iv[i]; i += x) {
                if (iv[i].async && iv[i].cbCount < iv[i].pulse.repeat) {
                    return; // need to wait for pending async callbacks
                } else if (i === 0 || !iv[i].async) {
                    if (iv[i].cbCount < iv[i].pulse.repeat) { // start emitting next in line
                        return emit(pw, iv, iv[i]);
                    } else if (x < 0) { // look at next emmited event
                        i = ci;
                        x = 1;
                    }
                }
...
function driplet(pw, iv, evt) {
    var pulse = typed(merge({ event: typeof evt !== 'object' ? evt : null, repeat: 1 }, evt, true, true, true), iv.type);
 ...
 * @param ctyp boolean that ensures that source values are constrained to the same type as the destination values when present
