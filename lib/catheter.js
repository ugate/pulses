...
function catheter(pw, args) {
     ...
     * @arg {Boolean} [exit] pass true (and omit drip) to prevent continuation of event drainage after listeners have been removed
     */
    ...
    iv.flow = {
        pause: function pause() {
            if (arguments.length) return (iv.stases = iv.stases || []).push(arguments);
        },
        resume: function resume() {
            if (iv.stases) for (var i = 0; i < iv.stases.length; i++) iv.stases[i]();
            iv.stases = null;
        }
    };
...
function Pulse(pw, iv, drip) {
    var pulse = this;
    plet.props(pulse, sprops, drip.pulse, true, true);
...
function Drip(pw, iv, evt, endEvent, emit) {
    ...
    this.pulse = plet.props(plet.props(pulse, sprops, eo, false, true), sprops, iv);
    this.cbCount = 0;
 ...
 * @arg {Array} iv the I.V. catheter
 */
Drip.prototype.emit = function emit(pw, iv) {
    var drip = this;
    if (iv.stases) return iv.stasis(function eflux() {
        drip.emit(pw, iv);
    });
    ...
    var pulse = new Pulse(pw, iv, drip), m = drip.pulse.type === 'async' ? pw.emitAsync : pw.emit;
    var argsp = iv.pass.length ? iv.pass : null, argsg = iv.args && iv.args.length ? iv.args : null;
    iv.emitted.push(pulse);
    if (argsp || argsg) {
        var args = [pulse.event, iv.stasis, iv.artery, pulse];
        args = argsp && argsg ? args.concat(argsp, argsg) : args.concat(argsp || argsg);
        iv.pass.length = 0;
        m.apply(pw, args);
    } else m.call(pw, pulse.event, iv.stasis, iv.artery, pulse);
};
...
function valve(pw, iv, evt) {
    ...
    pw.at(drip.pulse.event, drip.callback);
