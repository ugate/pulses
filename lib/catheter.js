 ...
 * @arg {Object} [passByRef={}] the initial object containing key/values that will check that a key matches a pulse events "refId" before passing its value into the first listener in the chain (after artery and pulse)
 * @arg {*[]} [pass] the initial arguments that will be passed to the first listener in the chain (after "passByRef" arguments from a prior listener)
 * @arg {*[]} [args] the initial arguments that will be passed to all listeners in the chain (after any "pass" arguments from a prior listener)
 * @returns {Array} an I.V. array with corresponding catheter functions
 */
function catheter(pw, lfn, it, passByRef, pass, args) {
    var iv = [];
    iv.listen = lfn;
    iv.target = it || pw;
    iv.dripCount = 0;
    ...
    iv.artery = null;
    iv.data = [];
    iv.passByRef = passByRef && typeof passByRef === 'object' ? passByRef : {};
...
function artery(pw, iv, meds) {
    ...
    iv.artery.passByRef = iv.passByRef;
...
Drip.prototype.emit = function emit(pw, iv, noInbound) {
    ...
    if (argsp || argsg) {
        ...
        if (isPass && pulse.passRefNames && pulse.passRefNames.length) {
            var pbr = [];
            for (var rk in iv.passByRef) if (iv.passByRef.hasOwnProperty(rk) && !!~pulse.passRefNames.indexOf(rk)) {
                pbr.push(iv.passByRef[rk]);
            }
            pbr.sort(); // ensure alpabetic order of pass by reference arguments
            for (var i = 0, l = pbr.length; i < l; ++i) args.push(iv.passByRef[pbr[i]]);
        };
    ...
    return args ? m.apply(pw, args) : m.call(pw, pulse.event, flow, iv.artery, pulse);
};
