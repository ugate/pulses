 ...
 */
function catheter(pw, pass, args) {
    var iv = [];
    iv.dripCount = 0;
    iv.count = 1;
    iv.meds = [];
    ...
    iv.flow = {
         ...
         * @returns {Integer} the new length of the queue (undefined when nothing is pushed to the queue)
         */
        pause: function pause() {
            iv.stases = iv.stases || [];
            if (arguments.length) return iv.stases.push(arguments);
        },
...
Drip.prototype.emit = function emit(pw, iv) {
    var drip = this;
    if (iv.stases) return iv.pause(function eflux() {
        drip.emit(pw, iv);
    });
    ...
    if (argsp || argsg) {
        ...
        if (drip.pulse.count >= drip.pulse.repeat) iv.pass.length = 0; // auto-clear passed parameters
        m.apply(pw, args);
