...
function catheter(pw, args) {
     ...
     * 
     * @arg {Object} [drip=iv] the drip created during a pump that contains pulse event data
...
Drip.prototype.emit = function emit(pw, iv) {
    ...
    var pulse = new Pulse(pw, iv, this), m = this.pulse.type === 'async' ? pw.emitAsync : pw.emit;
    iv.emitted.push(pulse);
    if (iv.args && iv.args.length) {
        m.apply(pw, [pulse.event, iv.flow, iv.artery, pulse].concat(iv.args));
    } else {
        m.call(pw, pulse.event, iv.flow, iv.artery, pulse);
    }
};
